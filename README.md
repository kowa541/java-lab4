# java-lab4
Степина Алёна ИТ-2 Лабораторная №4
...
Создан класс Box<T> с приватным полем T value.
Метод put(T item):
проверяет, пуста ли коробка (value == null);
если не пуста — выбрасывает IllegalStateException;
иначе сохраняет значение.
Метод get():
возвращает текущее значение;
после извлечения обнуляет поле (value = null).
Метод isEmpty() возвращает true, если коробка пуста.
...
Определён функциональный интерфейс ComparableItem<T> с методом:
int compare(T other);

Интерфейс может быть реализован любым классом, требующим сравнения.
Поддерживает параметризацию, не зависящую от конкретного типа.
...
Создан статический метод:
public static double findMax(List<Box<? extends Number>> boxes)

Использован wildcard ? extends Number для поддержки Integer, Double, Float и др.
Из каждой коробки извлекается значение через box.get().doubleValue().
Максимум находится через Math.max().
...
Создан кастомный интерфейс Function<T, R> с методом apply(T t).
Реализован метод:
public static <T, R> List<R> map(List<T> list, Function<T, R> func)

Для каждого элемента вызывается func.apply(item), результат добавляется в новый список.
...
Создан интерфейс Predicate<T> с методом test(T t).
Метод:
public static <T> List<T> filter(List<T> list, Predicate<T> pred)

возвращает только те элементы, для которых pred.test(item) == true.
...
Создан интерфейс BinaryOperator<T> с методом apply(T a, T b).
Метод:
public static <T> T reduce(List<T> list, T identity, BinaryOperator<T> op)

использует начальное значение (identity) и бинарную операцию.
...
Использованы:
Supplier<C> — для создания коллекции;
Consumer<T> — для добавления элемента.
Метод:
